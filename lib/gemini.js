...
function fallbackProductName(message) {
  return String(message || "")
    .replace(/["'`]/g, "")
    .replace(/\b(find|show|search|buy|get|price|for|of|me|please|daraz|on|under|below|above)\b/gi, " ")
...
`;
  ...
  try {
    ...
    const exactQuery = String(parsed.exactQuery || "").trim() || fallback.exactQuery;
    const normalizedQuery = String(parsed.normalizedQuery || exactQuery).trim() || fallback.normalizedQuery;
    const relatedHints = Array.isArray(parsed.relatedHints)
      ? parsed.relatedHints.map((q) => String(q || "").trim()).filter(Boolean).slice(0, 3)
